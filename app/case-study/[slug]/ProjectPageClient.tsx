    ...
    const ctx = gsap.context(() => {
      ...
      sectionRefs.current.forEach((section) => {
        ...
        images.forEach((img) => {
          gsap.fromTo(
            img,
            { x: (img as HTMLElement).classList.contains('image-left') ? -80 : 80, opacity: 0 },
            {
              x: 0,
              opacity: 1,
              duration: 1,
    ...
    <main ref={heroRef} className="min-h-screen bg-white">
      ...
      <section className="relative min-h-[75vh] flex items-center justify-center bg-black text-white overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 sm:px-10 py-20 text-center">
          ...
          <h1 
            ref={titleRef}
            className="text-5xl sm:text-7xl lg:text-8xl font-black mb-10 tracking-tighter leading-[0.9]"
      ...
      {/* AI SEO Optimized Project Overview / Quick Insight */}
      <section className="py-16 md:py-24 bg-black border-y border-white/5 relative">
        <div className="absolute inset-0 opacity-10">
          <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(circle_at_center,#10b98115_0%,transparent_70%)]" />
        </div>
        <div className="max-w-7xl mx-auto px-6 sm:px-10 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 lg:divide-x lg:divide-white/10">
            {/* The Problem / Challenge */}
            <div className="space-y-4">
              <h3 className="text-xs uppercase tracking-widest text-emerald-500 font-bold">The Challenge</h3>
              <p className="text-gray-300 leading-relaxed text-sm md:text-base">
                Modernizing {project.title} required overcoming workflow bottlenecks and implementing a scalable, AI-ready architecture for enterprise-grade performance.
              </p>
            </div>
            
            {/* The Solution */}
            <div className="lg:pl-12 space-y-4">
              <h3 className="text-xs uppercase tracking-widest text-blue-500 font-bold">The Solution</h3>
              <p className="text-gray-300 leading-relaxed text-sm md:text-base">
                We deployed a custom {project.tags[0]} infrastructure utilizing {project.expertise[0]} to automate processes and improve data-driven decision making.
              </p>
            </div>
            
            {/* Impact Metrics */}
            <div className="lg:pl-12 space-y-6">
              <h3 className="text-xs uppercase tracking-widest text-purple-500 font-bold">Key Benchmarks</h3>
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-white/5 rounded-xl border border-white/10">
                  <div className="text-xl md:text-2xl font-bold text-white">~60%</div>
                  <div className="text-[10px] text-gray-500 uppercase tracking-tight">Efficiency Gain</div>
                </div>
                <div className="p-4 bg-white/5 rounded-xl border border-white/10">
                  <div className="text-xl md:text-2xl font-bold text-white">4.2x</div>
                  <div className="text-[10px] text-gray-500 uppercase tracking-tight">Scale Factor</div>
                </div>
              </div>
            </div>
          </div>
                ...
                <div className="relative h-full min-h-[140px] p-6 rounded-2xl bg-white/5 border border-white/10 hover:border-white/20 transition-all duration-500 hover:bg-white/10 backdrop-blur-sm overflow-hidden flex flex-col justify-between">
                  ...
                  <div className="relative z-10">
                    <div className="flex items-start gap-3">
                      <div className="w-2 h-2 rounded-full bg-white/40 group-hover:bg-white mt-2 shrink-0 transition-colors duration-300" />
                      <h3 
