...
export default function BlogListContent({ blogs: initialBlogs, categoriesData }: BlogListContentProps) {
	const [selectedCategory, setSelectedCategory] = useState({ id: 'ALL', name: 'ALL' })
	const [isDropdownOpen, setIsDropdownOpen] = useState(false)
	const [categories] = useState([{ id: 'ALL', name: 'ALL' }, ...categoriesData.map((c: any) => ({ id: c._id, name: c.name }))])
	const [blogPosts] = useState(initialBlogs)
	...
	const filteredBlogs = blogPosts.filter((post: any) => {
		const matchesSearch =
			...
			(post.metaDescription?.toLowerCase().includes(searchQuery.toLowerCase()) || false)

		const matchesCategory = selectedCategory.id === 'ALL' || (post.category && (post.category._id === selectedCategory.id || post.category === selectedCategory.id))
